...
setup() {
    ...
    for d in "$T1" "$T2" "$T3" "$T4" "$T5" "$INSIDE" "$T7" "$T8"; do
        if [ -e "$d" ]; then
...
verify() {
    ...
    if [ "$n" != "8" ]; then
        echo "warn this round wrote $n hostselftest lines, not 8. A missing line shifts the" >&2
        echo "     positions below: every case is checked by its own message or its own bytes," >&2
        echo "     so a shift shows up in all eight." >&2
    fi
    ...
    echo
    echo "evidence (last 8 of $n hostselftest lines this round, log offset $offset):"
    printf '%s\n' "$out" | sed 's/^/  /'

    echo
    if [ "$c1" = PASS ] && [ "$c2" = PASS ] && [ "$c3" = PASS ] && [ "$c4" = PASS ] &&
        [ "$c5" = PASS ] && [ "$c6" = PASS ] && [ "$c7" = PASS ] && [ "$c8" = PASS ]; then
        echo "all eight PASS."
        return 0
    fi
